...
RunWorkspace 建立本身也属于事务边界。若真实项目在复制 snapshot 的短窗口内发生变化，CWO
可以在同一 bounded invocation 中重新获取 snapshot；只有稳定的 snapshot 才会启动 Agent。若
变化持续存在，CWO 必须保守地停止并保留 setup evidence，而不能把不完整 workspace、候选文件
或并发写入解释为成功。
