...
- 中断 recovery 在 workspace 已由先前的受支持 recovery 丢弃、且已保存 scoped drift 证据时
  可幂等重入；不会把已核验的 recovery 证据误报成 metadata 不完整，也不会改变 authority 或
  candidate 语义。
...
- `ARTIFACT_REVIEW` 与 `ARTIFACT_PATCH` invocation 会把 external candidate 物化到该
  ...
  文件而误判 candidate provenance。若 candidate 声明受约束的 linked projection，semantic
  review workspace 还会物化同一组经 hash 校验的 linked files；reviewer 直接运行 project
  validator 时不会看到 real-project baseline 与 candidate bundle 混合的假失败。Patch workspace
  仍只把主 candidate 作为可变 artifact scope，避免扩大 Agent 写权限。
