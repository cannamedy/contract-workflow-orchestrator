...
* `RunWorkspace` snapshots the current project view outside the target repository for every Agent invocation. `AgentRunner` receives only that workspace as cwd; `MockRunner` is a formal test path, `CodexCliRunner` is the live CLI adapter, and `CodexAppServerRunner` is an intentional future extension seam.
...
* `GitRemoteAuthorityWatcher` discovers the configured Git remote and branch, fetches into an external bare cache, compares the exact configured Human Guide blob/content, and hands only a new submitted revision to the existing Authority Change Intake.
...
* `EngineeringArtifact` provides an optional typed artifact DAG. Generic generation/review/patch stages carry the artifact id and kind while skills provide content methods; deterministic CWO code owns lifecycle, dependency closure, candidate persistence, validator hooks, and `APPROVED → PROMOTION_READY → ACCEPTED` promotion according to `AUTO`, `HUMAN_GATE`, or `EXTERNAL` policy.
* Project-mutating stages produce a workspace diff. CWO validates the outcome, task scope, authority protection, real-project baseline, and Git state before transactionally applying only the validated file set. Promotion is deterministic CWO code and does not delegate authority writes to an Agent.
...
The local Human Guide is not an authority-change trigger. Remote commit movement is only a fetch hint; the exact configured path's Git blob and content SHA-256 determine whether a submitted authority revision exists. Remote candidates are snapshotted under external state and carry base/candidate commit and blob provenance. Fetch or DNS failures preserve the previous ledger and do not stop unrelated work.

Recovery reads only `state.json`, run artifacts, the current workflow, Git, and project traceability artifacts. It never relies on previous Codex conversation memory, hidden context, or model memory. A valid outcome left behind before a state transition is reconciled idempotently only after its completed workspace and real-project baseline pass validation. An unfinished invocation with unknown side effects never commits its workspace and enters `HARD_STOP` with `RECOVERY_UNCERTAIN`.
