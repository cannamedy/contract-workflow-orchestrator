...
An optional `artifact_pipeline` turns artifact production into a typed DAG. Supported kinds are `HUMAN_GUIDE`, `ENGINEERING_SPEC`, `MACHINE_CONTRACT`, `CONFORMANCE_SPEC`, `IMPLEMENTATION_DESIGN`, `IMPLEMENTATION_PLAN`, `PLAN_GRAPH`, and `TASK_CONTRACT`. CWO owns artifact identity, lifecycle, dependency scheduling, candidate isolation, promotion preconditions, and validation hooks; the configured `skill_role` owns artifact content. Generic `ARTIFACT_GENERATION`, `ARTIFACT_VALIDATION`, `ARTIFACT_REVIEW`, and `ARTIFACT_PATCH` stages preserve candidate/accepted separation. A configured `project_validators` invocation runs in a read-only Run Workspace against the exact materialized candidate; semantic review receives that same exact linked-file candidate projection. Only structured `PASS` or `PASS_WITH_WARNINGS` validator evidence can reach semantic review. Deterministic validator failures route to patch, while invocation failures remain infrastructure failures. Each artifact declares `promotion_policy: AUTO`, `HUMAN_GATE`, or `EXTERNAL`; accepted revisions retain provenance and upstream revision hashes. Workflows without this configuration continue through the legacy Contract/Plan adapter.
